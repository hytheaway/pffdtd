...
def rotmatrix_ax_ang(Rax: Any, Rang: float):
    ...
    R = np.array(
        [
            [
                cos(theta) + Rax[0] * Rax[0] * (1 - cos(theta)),
                Rax[0] * Rax[1] * (1 - cos(theta)) - Rax[2] * sin(theta),
                Rax[0] * Rax[2] * (1 - cos(theta)) + Rax[1] * sin(theta),
            ],
            [
                Rax[1] * Rax[0] * (1 - cos(theta)) + Rax[2] * sin(theta),
                cos(theta) + Rax[1] * Rax[1] * (1 - cos(theta)),
                Rax[1] * Rax[2] * (1 - cos(theta)) - Rax[0] * sin(theta),
            ],
            [
                Rax[2] * Rax[0] * (1 - cos(theta)) - Rax[1] * sin(theta),
                Rax[2] * Rax[1] * (1 - cos(theta)) + Rax[0] * sin(theta),
                cos(theta) + Rax[2] * Rax[2] * (1 - cos(theta)),
            ],
        ]
    )
...
def rotate_xyz_deg(thx_d, thy_d, thz_d):
    ...
    Rx = npa([[1, 0, 0], [0, np.cos(thx), -np.sin(thx)], [0, np.sin(thx), np.cos(thx)]])

    Ry = npa([[np.cos(thy), 0, np.sin(thy)], [0, 1, 0], [-np.sin(thy), 0, np.cos(thy)]])

    Rz = npa([[np.cos(thz), -np.sin(thz), 0], [np.sin(thz), np.cos(thz), 0], [0, 0, 1]])
...
def vclamp(x, xmin, xmax):
    return np.where(
        x < xmin, xmin, np.where(x > xmax, xmax, x)
...
def wavwrite(fname, SR, data):
    ...
    scipy.io.wavfile.write(
        fname, int(SR), np.float32(data.T)
