...
class SimEngine:
    def __init__(self, data_dir, energy_on=False, nthreads=None):
        ...
        max_threads = int(getattr(nb.config, "NUMBA_MAX_THREADS", 1) or 1)
        nthreads = max(1, min(int(nthreads), max_threads))
        self.print(f"numba set for {nthreads=} (max {max_threads})")
    ...
    def set_coeffs(self):
        ...
        if self.energy_on:
            ...
            self.E_bnl = np.copy(
                mat_coeffs_struct[mat_bnl]["E"]
            )  # this picks up fake rigid (zeros)
    ...
    def run_all(self, nsteps=1):
        ...
        self.print(
            f"Run-time loop: {t_elapsed:.6f}, {Nt * Npts / 1e6 / t_elapsed:.2f} MVox/s"
        )
    ...
    def run_plot(self, nsteps=1, draw_backend="mayavi", json_model=None):
        ...
        if draw_backend == "matplotlib":
            ...
            plt.plot(
                xy_x.flat[bnm_xy.T.flat[:]],
                xy_y.flat[bnm_xy.T.flat[:]],
                marker=".",
                markersize=ms,
                linestyle="none",
                color=co,
            ...
            plt.plot(
                xz_x.flat[bnm_xz.T.flat[:]],
                xz_z.flat[bnm_xz.T.flat[:]],
                marker=".",
                markersize=ms,
                linestyle="none",
                color=co,
            ...
            plt.plot(
                yz_y.flat[bnm_yz.T.flat[:]],
                yz_z.flat[bnm_yz.T.flat[:]],
                marker=".",
                markersize=ms,
                linestyle="none",
                color=co,
            ...
            if json_model is not None:
                ...
                for mat in mat_str:
                    ...
                    for j in range(3):
                        ii = np.nonzero(
                            (pts[tris[:, j % 3], 2] > zv[iz_in])
                            ^ (pts[tris[:, (j + 1) % 3], 2] > zv[iz_in])
                        )[0]
                        edges_xy_0 = np.r_[edges_xy_0, pts[tris[ii, j % 3]][:, [0, 1]]]
                        edges_xy_1 = np.r_[
                            edges_xy_1, pts[tris[ii, (j + 1) % 3]][:, [0, 1]]
                        ]
                        del ii

                        ii = np.nonzero(
                            (pts[tris[:, j % 3], 1] > yv[iy_in])
                            ^ (pts[tris[:, (j + 1) % 3], 1] > yv[iy_in])
                        )[0]
                        edges_xz_0 = np.r_[edges_xz_0, pts[tris[ii, j % 3]][:, [0, 2]]]
                        edges_xz_1 = np.r_[
                            edges_xz_1, pts[tris[ii, (j + 1) % 3]][:, [0, 2]]
                        ]
                        del ii

                        ii = np.nonzero(
                            (pts[tris[:, j % 3], 0] > xv[ix_in])
                            ^ (pts[tris[:, (j + 1) % 3], 0] > xv[ix_in])
                        )[0]
                        edges_yz_0 = np.r_[edges_yz_0, pts[tris[ii, j % 3]][:, [1, 2]]]
                        edges_yz_1 = np.r_[
                            edges_yz_1, pts[tris[ii, (j + 1) % 3]][:, [1, 2]]
                        ]
                        del ii
                    ...
                    mlab.triangular_mesh(
                        *(pts.T), tris, color=(1.0, 1.0, 1.0), opacity=0.2
                    )
            ...
            hh_xy = mlab.mesh(
                xy_x,
                xy_y,
                np.full(xy_x.shape, zv[iz_in]),
                scalars=ldraw(uxy.T),
                colormap=cmap,
            )
            hh_xz = mlab.mesh(
                xz_x,
                np.full(xz_x.shape, yv[iy_in]),
                xz_z,
                scalars=ldraw(uxz.T),
                colormap=cmap,
            )
            hh_yz = mlab.mesh(
                np.full(yz_y.shape, xv[ix_in]),
                yz_y,
                yz_z,
                scalars=ldraw(uyz.T),
                colormap=cmap,
            ...
            mcb_xy = mlab.colorbar(
                object=hh_xy, orientation="vertical"
            )  # not sure if global scaling
            mcb_xz = mlab.colorbar(
                object=hh_xz, orientation="vertical"
            )  # not sure if global scaling
            mcb_yz = mlab.colorbar(
                object=hh_yz, orientation="vertical"
            )  # not sure if global scaling

            mlab.orientation_axes()

            fake_verts = np.c_[
                npa([xv[0], yv[0], zv[0]]), npa([xv[-1], yv[-1], zv[-1]])
            ]
            mlab.plot3d(*fake_verts, transparent=True, opacity=0)
            mlab.axes(xlabel="x", ylabel="y", zlabel="z", color=(0.0, 0.0, 0.0))
    ...
    def run_steps(self, nstart, nsteps):
        ...
        for n in range(nstart, nstart + nsteps):
            if energy_on:
                ...
                H_tot[n] = (
                    V_fac * 0.5 * h * nb_energy_int(u1, u2, Lu2, l2)
                )  # H_tot[n] = V_fac*0.5*h*np.sum((((u1-u2)**2)/l2 - u1*Lu2)[1:Nx-1,1:Ny-1,1:Nz-1])
                H_tot[n] -= (
                    V_fac
                    * 0.5
                    * h
                    * np.sum(
                        (1.0 - V_bna)
                        * (
                            ((u1.flat[bna_ixyz] - u2.flat[bna_ixyz]) ** 2) / l2
                            - u1.flat[bna_ixyz] * Lu2.flat[bna_ixyz]
                        )
                    )
                )
                ...
                H_tot[n] += (
                    ...
                    * c
                    / l2
                    * nb_energy_stored(ssaf_bnl, vh1, D_bnl, gh1, F_bnl, Ts)
                )  # H_tot[n] += V_fac*0.5*c/l2*np.sum(ssaf_bnl*((vh1**2)*D_bnl + ((Ts*gh1)**2)*F_bnl).T)
            ...
            if energy_on:
                E_lost[n + 1] = (
                    E_lost[n]
                    + V_fac * 0.25 * h / l * nb_energy_loss(ssaf_bnl, vh0, vh1, E_bnl)
                )  # E_lost[n+1] = E_lost[n] + V_fac*0.25*h/l*np.sum(ssaf_bnl*(((vh0+vh1)**2)*E_bnl).T)
                ...
                E_lost[n + 1] += (
                    ...
                    * h
                    / l
                    * np.sum((V_bna * Q_bna) * (u0.flat[bna_ixyz] - u2ba) ** 2)
                )  # E_lost[n+1] += 0.5*V_fac*h/l*nb_energy_loss_abc(V_bna,Q_bna,u0,u2ba,bna_ixyz) #problem with numba fn signature
                # H_tot[n] += E_lost[n]

                E_in[n + 1] = E_in[n] + (V_fac * h / l2) * 0.5 * np.sum(
                    (u0.flat[in_ixyz] - u2in) * in_sigs[:, n]
                )  # have to undo (l2/h/V_fac) scaling applied to in_sigs
    ...
    def print_last_energy(self, Np):
        ...
        for n in range(Nt - Np, Nt):
            self.print(
                f"normalised energy balance:{rel_diff(H_tot[n] + E_lost[n], E_in[n]):.16e}"
            )
...
def nb_leapfrog_update(u0, u1, Lu1, l2):
    ...
    for ix in nb.prange(1, Nx - 1):
        for iy in range(1, Ny - 1):
            for iz in range(1, Nz - 1):
                u0[ix, iy, iz] = (
                    2.0 * u1[ix, iy, iz] - u0[ix, iy, iz] + l2 * Lu1[ix, iy, iz]
                )
...
@nb.jit(nopython=True, parallel=True)
def nb_update_bnl_fd(
    u0, u2b, l, bnl_ixyz, ssaf_bnl, vh0, vh1, gh1, mat_bnl, mat_coeffs_struct
):
    Nbl = bnl_ixyz.size
...
def nb_energy_int(u1, u2, Lu2, l2):
    Nx, Ny, Nz = u1.shape
    return np.sum(
        (((u1 - u2) ** 2) / l2 - u1 * Lu2)[1 : Nx - 1, 1 : Ny - 1, 1 : Nz - 1]
    )
    # psum = 0.0
...
def main():
    ...
    parser.add_argument(
        "--nsteps", type=int, help="run in batches of steps (less frequent progress)"
    )
    parser.add_argument(
        "--nthreads", type=int, help="number of threads for parallel execution"
    )
